...
export class Metric extends React.Component<IMetricProps> {
  ...
  render() {
    ...
    return (
      <div>
        <h2 id="metric-heading">
          Metrics
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp; Create new Metric
          </Link>
        </h2>
            ...
            <tbody>
              {metricList.map((metric, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{metric.termType}</td>
                  <td>{metric.term}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${metric.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${metric.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${metric.id}/delete`} color="danger" size="sm">
